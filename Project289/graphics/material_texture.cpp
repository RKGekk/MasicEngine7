...
MaterialTexture::MaterialTexture(ID3D11Device* device, const std::string& fileName) : MaterialTexture(device, fileName, TextureType::UNKNOWN){}

MaterialTexture::MaterialTexture(ID3D11Device* device, const MaterialColor& color, TextureType type) {
	InitializeColorMaterial(device, color, type);
}

MaterialTexture::MaterialTexture(ID3D11Device* device, const MaterialColor* color_data, UINT width, UINT height, TextureType type) {
	InitializeTextureMaterial(device, color_data, width, height, type);
}

MaterialTexture::MaterialTexture(ID3D11Device* device, const std::string& fileName, TextureType type) : m_type(type) {
...
MaterialTexture::MaterialTexture(ID3D11Device* device, const uint8_t* pData, size_t size, TextureType type) {
	m_type = type;
...
void MaterialTexture::InitializeColorMaterial(ID3D11Device* device, const MaterialColor& color, TextureType type) {
	InitializeTextureMaterial(device, &color, 1, 1, type);
}

void MaterialTexture::InitializeTextureMaterial(ID3D11Device* device, const MaterialColor* color_data, UINT width, UINT height, TextureType type) {
	m_type = type;
