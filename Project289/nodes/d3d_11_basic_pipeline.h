...
public:
	D3D11BasicPipeline(const MeshHolder& mesh_data, std::shared_ptr<PixelShader> ps, std::shared_ptr<VertexShader> vs, const DirectX::XMFLOAT4X4* pMatrix);
	D3D11BasicPipeline(const MeshHolder& mesh_data, std::shared_ptr<PixelShader> ps, std::shared_ptr<VertexShader> vs, DirectX::FXMMATRIX to, DirectX::CXMMATRIX from, bool calulate_from = false);
...
protected:
	...
	ID3D11ShaderResourceView* const* m_srv0;
