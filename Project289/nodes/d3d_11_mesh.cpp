...
D3D11Mesh::D3D11Mesh(int mesh_id, BaseRenderComponent* renderComponent, DirectX::FXMMATRIX to, DirectX::CXMMATRIX from, bool calculate_from) : D3D11Drawable(nullptr, to, from, calculate_from) {
	...
	const MeshHolder& mesh = mrc->GetMesh(mesh_id);

	D3DRenderer11* renderer = static_cast<D3DRenderer11*>(g_pApp->GetRenderer());
	ID3D11Device* device = renderer->GetDevice();

	const Vertices& vertices = mesh.GetVertices();
	std::unique_ptr<VertexBufferBindable> vb = std::make_unique<VertexBufferBindable>(device, vertices);
	AddBind(BindableType::vertex_buffer_bindable, std::move(vb));

	const Indices& indices = mesh.GetIndices();
	std::unique_ptr<IndexBufferBindable> ibuf = std::make_unique<IndexBufferBindable>(device, indices);
	...
	std::unique_ptr<ShaderResourceBindable> srv0 = std::make_unique<ShaderResourceBindable>(device, mesh.GetFirstTexture(TextureType::DIFFUSE).GetTextureResourceViewAddress());
