...
D3D11BasicPipeline::D3D11BasicPipeline(const MeshHolder& mesh_data, std::shared_ptr<PixelShader> ps, std::shared_ptr<VertexShader> vs, const DirectX::XMFLOAT4X4* pMatrix) : D3D11BasicPipeline(mesh_data, ps, vs, DirectX::XMLoadFloat4x4(pMatrix), DirectX::XMMatrixIdentity(), true) {}

D3D11BasicPipeline::D3D11BasicPipeline(const MeshHolder& mesh_data, std::shared_ptr<PixelShader> ps, std::shared_ptr<VertexShader> vs, DirectX::FXMMATRIX to, DirectX::CXMMATRIX from, bool calulate_from) : SceneNode(nullptr, RenderPass::RenderPass_Actor, to, from, calulate_from) {
	D3DRenderer11* renderer = static_cast<D3DRenderer11*>(g_pApp->GetRenderer());
	ID3D11Device* device = renderer->GetDevice();

	const Vertices& vertices = mesh_data.GetVertices();
	...
	vbd.MiscFlags = 0u;
	vbd.ByteWidth = UINT(m_vertex_stride * vertices.size());
	...
	const Indices& indices = mesh_data.GetIndices();
	...
	m_srv_start_slot = 0;
	m_srv0 = mesh_data.GetFirstTexture(TextureType::DIFFUSE).GetTextureResourceViewAddress();
